...
function accounting(training_time, depth)
  ...
  if opt.dataset == 'svhn' then 
    ...
    final:write(string.format('Epoch %d:\t%.2f%%\t\t%.2f%%\t\t%0.0fs\t\t%d\t\t%d\t\t%d\n', 
       sgdState.epochCounter, results[1]*100, results[2]*100, training_time, 
       depth['depth_G1'], depth['depth_G2'], depth['depth_G3']))
  end
end
...
function train(depth, counter, timer)  
  depth1_suggestion = depth['depth_G1'] or 0
  depth2_suggestion = depth['depth_G2'] or 0
  depth3_suggestion = depth['depth_G3'] or 0
  ...
  --while sgdState.epochCounter <= opt.maxEpochs do
    ...
    for i=1,#batches do
        ...
        elseif opt.deathMode == 'spearmint' then
            for i,tb in ipairs(addtables) do
              if i <= opt.N then
                if i%opt.N >= depth1_suggestion then model:get(tb).gate = false end
              elseif i <= 2*opt.N then
                if i%opt.N >= depth2_suggestion then model:get(tb).gate = false end 
              else
                if i%opt.N >= depth3_suggestion then model:get(tb).gate = false end
              end
            end
        end
        function feval(x)
        ...
        if opt.dataset == 'svhn' then
          if sgdState.iterCounter % 200 == 0 then
            accounting(timer:time().real, depth)
            timer:reset()
          end
