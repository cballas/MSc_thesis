...
opt = lapp[[
  --maxEpochs     (default 1300)         Maximum number of epochs to train the network
  ...
  --deathMode     (default lin_decay)   Use lin_decay or uniform or spearmint
...
if opt.dataset == "cifar10" or opt.dataset == "cifar100" or opt.dataset == "svhn" then
    ...
    for i=1,5 do   addResidualDrop(model, nil, 16)   end
    addResidualDrop(model, nil, 16)
    ------> 32, 16,16   Second Group
    addResidualDrop(model, nil, 16, 32, 2)
    for i=1,18-1 do   addResidualDrop(model, nil, 32)   end
    ------> 64, 8,8     Third Group
    addResidualDrop(model, nil, 32, 64, 2)
    for i=1,8-1 do   addResidualDrop(model, nil, 64)   end
...
function main()  
  ...
  while sgdState.epochCounter <= opt.maxEpochs do
    ...
    elseif sgdState.epochCounter < opt.maxEpochs*lrSchedule[opt.dataset][3] then
      sgdState.learningRate = 0.001
    else
      sgdState.learningRate = 0.0001
    end
    ...
    for i=1,#batches do
        ...
        elseif opt.deathMode == 'spearmint' then
            depth_suggestion = HPOptim.find(10)
            for i,tb in ipairs(addtables) do
              if i > depth_suggestion['depth'] then model:get(tb).gate = false end
            end
        end
        function feval(x)
            gradients:zero()
